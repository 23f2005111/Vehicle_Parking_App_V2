...
def export_user_parking_csv(user_id):
    ...
    csv_file_name = f"parking_report_{user_id}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.csv"   # creating a unique filename
    ...
    with open(csv_path, 'w', newline="") as csvfile:
        ...
        for r in reservations:
            ...
            parking_csv.writerow(this_row) # creates info into csv format
            sr_no += 1
...
def monthly_report():
    ...
    for user in users:
        # Get user's reservations from the month 
...
def daily_parking_reminder():
    ...
    for user in users:
        ...
        if not active_reservation:
            ...
            print(f"📨 [DEMO] Reminder for {user.username}: {text}")
            reminder_count += 1
